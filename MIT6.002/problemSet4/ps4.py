...
path = "C:/Users/Scott/Documents/Coursework/pset4"
os.chdir(path)
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        degree = len(model)
        e_vals = (np.polyval(model, x))
        r_sqr = r_squared(y, e_vals)
        print (r_sqr)
        pylab.figure()
        pylab.plot(x, y, "bo", x, e_vals, "r-")
        pylab.title("Model of %d degrees with R^2: %d" % (degree, float(r_sqr)))
        pylab.xlabel("Year")
        pylab.ylabel("Temp")
        pylab.show()
